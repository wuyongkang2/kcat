...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" placeholder="" id="pw" name="pw">
		</div>
	</div>
	<div class="row cl">
...
$(function(){
	...
	var title = decodeURI("<%=request.getParameter("title")%>");
	var images = decodeURI("<%=request.getParameter("images")%>");
	var link = decodeURI("<%=request.getParameter("link")%>");
	var pw = decodeURI("<%=request.getParameter("pw")%>");
	...
	$("#form-admin-add").validate({
		...
		submitHandler:function(form){
			$.post("${pageContext.request.contextPath}/updateHomework.do",{id:id,cid:$('#category option:selected').val(),category:$('#category option:selected').text(),title:$("#title").val(),images:time_name2,link:$("#link").val(),pw:$('#pw').val()},function(data){
				if(data){
					parent.layer.msg('修改成功',{time: 500, icon: 1},function(){
						window.parent.location="${pageContext.request.contextPath}/admin/homework_list.jsp";
