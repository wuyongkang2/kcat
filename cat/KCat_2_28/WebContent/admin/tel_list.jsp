...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 主页 <span class="c-gray en">&gt;</span> 内容管理 <span class="c-gray en">&gt;</span> 校园电话 <span class="c-gray en">&gt;</span> 查看列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="soft_add('添加','tel_add.jsp','800','460')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a> </span> <span class="r">共有数据：<strong id="soft_count">加载中...</strong> 条</span> </div>
	<div class="mt-20">
...
$(function(){
	...
	$.ajaxSetup({async:false});
 	$.post("${pageContext.request.contextPath}/getAllTel.do",function(data){
 		$("#soft_count").text(data.length);
		$.each(data,function(i,e){
			html+="<tr class='text-c'><td><input type='checkbox' value='1' name=''></td><td>"+data[i].id+"</td><td>"+data[i].category+"</td><td>"+data[i].name+"</td><td><a onclick='datu("+"$(this)"+")'><img style='width:30px;height:30px;' src='http://kcat-1251241286.cosgz.myqcloud.com/images/"+data[i].images+"'</a></td><td>"+data[i].tel+"</td>"
...
function soft_edit(object){
	...
	var name = encodeURI(encodeURI(path.eq(3).text()));
	var images = encodeURI(encodeURI(path.eq(4).children().children()[0].src));
	var tel = encodeURI(encodeURI(path.eq(5).text()));
	member_edit('编辑','tel_modify.jsp?id='+id+'&category='+category+'&name='+name+'&images='+images+'&tel='+tel+'','800','460');
}
/*软件-编辑*/
...
function member_del(id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
	        ...
	        async : false, 
	        cache : false, 
	        url : "${pageContext.request.contextPath}/deleteTel.do",  
