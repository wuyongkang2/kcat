...
$(function(){
	...
	var videoType = decodeURI("<%=request.getParameter("videoType")%>");
	var videoImage = decodeURI("<%=request.getParameter("videoImage")%>");
	var videoExplain = decodeURI("<%=request.getParameter("videoExplain")%>");
	var videoUrl = decodeURI("<%=request.getParameter("videoUrl")%>");
	
	$("#videoName").val(videoName);
	
	$("#videoType option").each(function (){
	    if($(this).text()==videoType){   
	    $("#videoType").val($(this).val())  
	 }}); 
	
	time_name2 = sub(videoImage);
	$("#result2").val(sub(videoImage));
	$("#uploadFile_img2").attr('src',videoImage);
	jpg_flag = true;
	$("#videoExplain").val(videoExplain);
	...
	$("#form-admin-add").validate({
		...
		submitHandler:function(form){
			$.post("${pageContext.request.contextPath}/updateBang.do",{id:id,videoName:$("#videoName").val(),videoType:$('#videoType option:selected').val(),videoImage:time_name2,videoExplain:$("#videoExplain").val(),videoUrl:$('#videoUrl').val()},function(data){
				if(data){
